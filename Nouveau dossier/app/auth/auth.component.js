"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var alert_service_1 = require("../services/alert.service/alert.service");
var auth_service_1 = require("../services/auth.service/auth.service");
var AuthComponent = (function () {
    function AuthComponent(router, alertService, authService) {
        this.router = router;
        this.alertService = alertService;
        this.authService = authService;
        this.sideMenu = "question";
    }
    AuthComponent.prototype.changeMenuQuestion = function () {
        this.sideMenu = "question";
        console.log("sideMenu: " + this.sideMenu);
    };
    AuthComponent.prototype.changeMenuResult = function () {
        this.sideMenu = "result";
        console.log("sideMenu: " + this.sideMenu);
    };
    AuthComponent.prototype.signOut = function () {
        this.alertService.success('You have been logout sucessfully', true);
        this.authService.logout();
        this.router.navigate(['/login']);
    };
    return AuthComponent;
}());
AuthComponent = __decorate([
    core_1.Component({
        templateUrl: 'app/auth/auth.component.html'
    }),
    __metadata("design:paramtypes", [router_1.Router, alert_service_1.AlertService, auth_service_1.AuthService])
], AuthComponent);
exports.AuthComponent = AuthComponent;
//# sourceMappingURL=auth.component.js.map